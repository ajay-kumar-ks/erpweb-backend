...
async def run_async():
    ...
    # Summary for key areas
    key_paths = ['/api/accounts/coa', '/api/accounts/invoices', '/api/accounts/invoices/{id}/payments', '/api/accounts/bills/{id}/payments']
    summary = {'total': len(results), 'success': 0, 'fail': 0, 'details': {}}
    for r in results:
        ok = (r['status'] == 200)
        if ok:
            summary['success'] += 1
    ...
    def find(path_prefix):
        for r in results:
            if isinstance(r['path'], str) and r['path'].startswith(path_prefix):
                return r
        return None

    coa_r = find('/api/accounts/coa')
    inv_r = find('/api/accounts/invoices')
    inv_pay_r = None
    for r in results:
        if r['path'].startswith('/api/accounts/invoices/') and r['path'].endswith('/payments'):
            inv_pay_r = r
            break
    bill_pay_r = None
    for r in results:
        if r['path'].startswith('/api/accounts/bills/') and r['path'].endswith('/payments'):
            bill_pay_r = r
            break

    summary['details']['coa'] = coa_r['status'] if coa_r else None
    summary['details']['invoice_create'] = inv_r['status'] if inv_r else None
    summary['details']['invoice_payment'] = inv_pay_r['status'] if inv_pay_r else None
    summary['details']['bill_payment'] = bill_pay_r['status'] if bill_pay_r else None
